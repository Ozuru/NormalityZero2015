 ...
 *
 */
public class COTTONMOUTH_1 extends Command {

	public COTTONMOUTH_1() {
	...
	protected void execute() {
		if (!(Robot._forkLift.isBotSwitch() || Robot._forkLift.isTopSwitch())) {
			if (Robot._PRISM.getCurrentLevel() < 1) {
				Robot._forkLift.setPulley(RobotMap.FORKLIFT_RAISE_SPEED);
			} else if (Robot._PRISM.getCurrentLevel() > 1) {
	...
	protected boolean isFinished() {
		return (Robot._forkLift.isBotSwitch() || Robot._forkLift.isTopSwitch() //|| Robot._PRISM.isLevel1()
				);
	}
